...
class UIModel(BaseModel):
    ...
    def load_image(self, label_path, inst_path, feat_path):
        ...
        self.input_label = input_label.scatter_(
            1, label_map.long().cuda(), 1.0)
        ...
        if not opt.no_instance:
            ...
            self.net_input = Variable(
                torch.cat((self.input_label, self.edge_map), dim=1), volatile=True)
        else:
    ...
    def change_labels(self, click_src, click_tgt):
        ...
        if idx_src.shape:
            ...
            self.label_map[idx_src[:, 0], idx_src[:, 1],
                           idx_src[:, 2], idx_src[:, 3]] = label_tgt
            self.net_input[idx_src[:, 0], idx_src[:, 1] +
                           label_src, idx_src[:, 2], idx_src[:, 3]] = 0
            self.net_input[idx_src[:, 0], idx_src[:, 1] +
                           label_tgt, idx_src[:, 2], idx_src[:, 3]] = 1

            # update the instance map (and the network input)
            if inst_tgt > 1000:
                # if different instances have different ids, give the new object a new id
                tgt_indices = (self.inst_map > label_tgt *
                               1000) & (self.inst_map < (label_tgt+1) * 1000)
                inst_tgt = self.inst_map[tgt_indices].max() + 1
            self.inst_map[idx_src[:, 0], idx_src[:, 1],
                          idx_src[:, 2], idx_src[:, 3]] = inst_tgt
        ...
        self.fake_image = util.tensor2im(
            self.single_forward(self.net_input, self.feat_map))
    ...
    def add_strokes(self, click_src, label_tgt, bw, save):
        ...
        for i in range(bw):
            idx_src[i*bw:(i+1)*bw, 2] = min(h-1,
                                            max(0, click_src[0]-bw//2 + i))
            for j in range(bw):
            ...
            if idx.size():
                if style_id == -1:
                    ...
                    for cluster_idx in range(self.opt.multiple_output):
                        ...
                        fake_image = self.single_forward(
                            self.net_input, self.feat_map)
                        fake_image = util.tensor2im(
                            fake_image[:, min_y:max_y, min_x:max_x])
                    ...
                    if instToChange > 1000:
                        ...
                        for cluster_idx in range(self.opt.multiple_output):
                            ...
                            fake_image = \
                                util.tensor2im(fake_image[:,min_y-y_start:max_y-y_start,min_x-x_start:max_x-x_start])
                            self.fake_image.append(fake_image)
                    else:
                        ### downsample
    ...
    def set_features(self, idx, feat, cluster_idx):
        for k in range(self.opt.feat_num):
            self.feat_map[idx[:, 0], idx[:, 1] + k,
                          idx[:, 2], idx[:, 3]] = feat[cluster_idx, k]

    # copy the features at the target position to the source position
    def copy_features(self, idx_src, idx_tgt):
        for k in range(self.opt.feat_num):
            val = self.feat_map[idx_tgt[0],
                                idx_tgt[1] + k, idx_tgt[2], idx_tgt[3]]
            self.feat_map[idx_src[:, 0], idx_src[:, 1] +
                          k, idx_src[:, 2], idx_src[:, 3]] = val

    def get_current_visuals(self, getLabel=False):
        mask = self.mask
        if self.mask is not None:
            mask = np.transpose(
                self.mask[0].cpu().float().numpy(), (1, 2, 0)).astype(np.uint8)

        dict_list = [('fake_image', self.fake_image), ('mask', mask)]

        if getLabel:  # only output label map if needed to save bandwidth
            label = util.tensor2label(
                self.net_input.data[0], self.opt.label_nc)
